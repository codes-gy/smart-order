...
| Auth | ✅ 구현 완료 (`feature/gy/auth` 브랜치, 아직 미병합) | 소셜(카카오/애플, 토큰 서버 검증 포함)·SMS·매장관리자 로그인 + JWT 인프라 + `STORE_ADMIN` 인가(소유 매장 검증 포함) + 로그아웃 리프레시 토큰 무효화까지 구현. 남은 건 카카오/애플 실계정 발급, SMS 실발송 벤더 연동뿐(외부 계정 의존, 코드 경로는 준비됨). 상세 내역은 `backend/PROGRESS.md` 참고(로컬 CLI 세션이 관리하는 별도 문서) |
...
### [x] 1. 인증(Auth) 도메인 — `feature/gy/auth` 브랜치에서 완료(아직 이 브랜치엔 미병합, PR 준비 단계)
- `SecurityConfig.kt`의 TODO 해결: JWT 필터 추가 완료, 매장 관리 API(`PATCH /stores/**`)는 `STORE_ADMIN` 권한 + 소유 매장 검증으로 제한 완료.
- 필요 기능(`authApi.ts` 기준) 구현 완료: 소셜 로그인(카카오/애플 토큰 서버 검증 포함), SMS 인증, 매장 관리자 로그인, 리프레시 토큰(+로그아웃 시 무효화), 회원 탈퇴(soft-delete).
- 남은 것: 카카오 앱/애플 Services ID 실계정 발급 후 엔드투엔드 검증, SMS 실발송 벤더 연동 — 둘 다 외부 계정/과금 의존이라 코드 경로만 준비되고 로컬에서 실토큰 검증은 못함. `AuthControllerTest`/`AuthServiceTest` 외 통합 테스트 보강도 후속 우선순위. 이메일/비밀번호 플로우는 구현돼 있으나 프론트가 실제로 쓰지 않는 것으로 확인됨(3절 리스크 아님, `PROGRESS.md` 참고).
- 상세 구현 내역·의사결정 기록은 `backend/PROGRESS.md`(로컬 Claude Code CLI 세션 관리 문서) 참고 — 이 로드맵과 병행 관리되던 별도 문서라 여기엔 요약만 반영함.
